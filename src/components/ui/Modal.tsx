...
export default function Modal({ isOpen, onClose, title, icon, children }: ModalProps) {
  ...
  return (
    ...
    >
      ...
      {/* Modal Content */}
      <div
        className="relative w-full max-w-2xl max-h-[95vh] sm:max-h-[90vh] bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col transition-all duration-200"
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-[var(--card-border)]">
          ...
          <button
            onClick={onClose}
            className="p-1.5 sm:p-2 rounded-lg text-gray-400 hover:text-white hover:bg-[var(--card-border)] transition-colors"
