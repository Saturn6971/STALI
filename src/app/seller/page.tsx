...
type SellerListing = (System & { type: 'system' }) | (GPUListing & { type: 'gpu' });
...
export default function SellerDashboard() {
  ...
  const handleStatusChange = async (listingId: string, newStatus: 'active' | 'sold' | 'draft', listingType: 'system' | 'gpu') => {
    try {
      const tableName = listingType === 'system' ? 'systems' : 'gpu_listings';
  ...
  const handleDeleteListing = async (listingId: string, listingType: 'system' | 'gpu') => {
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-[var(--card-bg)] rounded-2xl p-6 border border-[var(--card-border)] text-center">
              <div className="text-3xl font-bold text-white mb-2">
                {systems.filter(s => s.status === 'active').length + gpuListings.filter(g => g.status === 'active').length}
              </div>
              <div className="text-gray-400">Active Listings</div>
            </div>
            <div className="bg-[var(--card-bg)] rounded-2xl p-6 border border-[var(--card-border)] text-center">
              <div className="text-3xl font-bold text-white mb-2">
                {systems.filter(s => s.status === 'sold').length + gpuListings.filter(g => g.status === 'sold').length}
              </div>
              <div className="text-gray-400">Sold Items</div>
            </div>
            <div className="bg-[var(--card-bg)] rounded-2xl p-6 border border-[var(--card-border)] text-center">
              <div className="text-3xl font-bold text-white mb-2">
                {systems.reduce((sum, s) => sum + (s.view_count || 0), 0) + gpuListings.reduce((sum, g) => sum + (g.view_count || 0), 0)}
              </div>
              <div className="text-gray-400">Total Views</div>
            </div>
            <div className="bg-[var(--card-bg)] rounded-2xl p-6 border border-[var(--card-border)] text-center">
              <div className="text-3xl font-bold text-white mb-2">
                {systems.reduce((sum, s) => sum + (s.favorite_count || 0), 0) + gpuListings.reduce((sum, g) => sum + (g.favorite_count || 0), 0)}
              </div>
      ...
      <section className="px-6 lg:px-12 py-4 border-b border-[var(--card-border)]">
        <div className="max-w-7xl mx-auto">
          <div className="flex space-x-1 bg-[var(--card-bg)] rounded-lg p-1">
            {[
              { id: 'active', label: 'Active', count: systems.filter(s => s.status === 'active').length + gpuListings.filter(g => g.status === 'active').length },
              { id: 'sold', label: 'Sold', count: systems.filter(s => s.status === 'sold').length + gpuListings.filter(g => g.status === 'sold').length },
              { id: 'draft', label: 'Drafts', count: systems.filter(s => s.status === 'draft').length + gpuListings.filter(g => g.status === 'draft').length }
            ].map((tab) => (
              <button
                key={tab.id}
                  ...
                  <div className="p-6">
                    ...
                    <div className="space-y-3">
                      <div className="flex space-x-2">
                        <Link
                          href={listing.type === 'system' ? `/systems/${listing.id}` : `/gpus/${listing.id}`}
                        ...
                        <Link
                          href={listing.type === 'system' ? `/seller/edit-listing/${listing.id}` : `/gpus/sell`}
