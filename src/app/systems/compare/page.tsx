...
import { SystemComparison } from '@/components/ui/SystemComparison';
import { System, supabase } from '@/lib/supabase';

function SystemCompareContent() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const [comparisonIds, setComparisonIds] = useState<string[]>([]);
  const [comparisonSystems, setComparisonSystems] = useState<System[]>([]);
  ...
  useEffect(() => {
    ...
    if (ids.length > 0) {
      fetchComparisonSystems(ids);
    } else {
      setLoading(false);
    }
  }, [searchParams]);

  const fetchComparisonSystems = async (ids: string[]) => {
    try {
      setLoading(true);
      // Fetch system listings for comparison
      ...
      if (error) {
        console.error('Error fetching comparison systems:', error);
        return;
      }

      setComparisonSystems(data || []);
    } catch (err) {
      console.error('Failed to fetch comparison systems:', err);
    } finally {
      setLoading(false);
    }
  };

  const removeFromComparison = (systemId: string) => {
    const newIds = comparisonIds.filter(id => id !== systemId);
    setComparisonIds(newIds);
    setComparisonSystems(comparisonSystems.filter(system => system.id !== systemId));
    
    // Update URL
    if (newIds.length > 0) {
      router.push(`/systems/compare?ids=${newIds.join(',')}`);
    } else {
      router.push('/systems');
    }
  };
